...
- Extension production code should import from `helios/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `helios.plugin.json` and the package
  `helios` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `helios/plugin-sdk/*`.
...
- When core needs plugin-owned static data on a hot path, expose a lightweight
  top-level artifact such as `helios-auth-api.ts`, `helios-tool-api.ts`, or a
