...
const ensureSupportedNodeVersion = () => {
  if (isSupportedNodeVersion(parseNodeVersion(process.versions.node))) {
    ...
    process.stderr.write(
      `helios: Node.js ${SUPPORTED_NODE_RANGE} is required (current: v${process.versions.node}).\n` +
      "If you use nvm, run:\n" +
...
const runRespawnedChild = (command, args, env) => {
  ...
  child.once("error", (error) => {
    detach();
    process.stderr.write(
      `[helios] Failed to respawn launcher: ${
...
const buildMissingEntryErrorMessage = async () => {
  const lines = ["helios: missing dist/entry.(m)js (build output)."];
  ...
  lines.push(
    "For pinned GitHub installs, use `npm install -g github:yourusername/helios#<ref>` instead of a raw `/archive/<ref>.tar.gz` URL.",
  );
  lines.push("For releases, use `npm install -g helios@latest`.");
  return lines.join("\n");
};
...
function tryOutputLauncherVersion(argv) {
  try {
    ...
    process.stdout.write(commit ? `Helios ${version} (${commit})\n` : `Helios ${version}\n`);
    return true;
  } catch {
    return false;
  }
